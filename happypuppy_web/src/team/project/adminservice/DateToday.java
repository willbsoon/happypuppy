...
public class DateToday {
	public Time_now getDate() {
		...
		String before3Monthstart = todayf.format(calendar.getTime());
		System.out.println("3개월전 1일 : " + before3Monthstart);
		//3개월전 마지막일
		calendar.add(Calendar.MONTH, +1); // 세달을 빼준다.
		calendar.add(Calendar.DAY_OF_YEAR, -1);// 25-24 = 1일로 만들어 준다.
		String before3Monthend = todayf.format(calendar.getTime());
		System.out.println("3개월전 마지막일 : " + before3Monthend);
		...
		//==AdminDao selectMonthly===========================================================
		calendar2.add(Calendar.MONTH, -1);
		String before2Month_start = today_f.format(calendar2.getTime());//yyyy-MM-dd
		calendar2.add(Calendar.MONTH, -1);
		String before3Month_start = today_f.format(calendar2.getTime());//yyyy-MM-dd
		calendar2.add(Calendar.MONTH, -1);
		String before4Month_start = today_f.format(calendar2.getTime());//yyyy-MM-dd
		calendar2.add(Calendar.MONTH, -1);
		String before5Month_start = today_f.format(calendar2.getTime());//yyyy-MM-dd
		
		//==AdminDao selectMonthly 끝===========================================================
		
		Time_now now = new Time_now(simpletoday, simple_today, 
				timetoday, simple_before, day, before1Month_start, 
				before2Month_start, before3Month_start, before4Month_start, 
				before5Month_start, before3Monthstart, before3Monthend);
		return now;
	}
}
