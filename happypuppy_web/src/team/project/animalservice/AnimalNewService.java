...
//최근 유기동물을 보여주는 서비스
public class AnimalNewService implements UserService {

	@Override
	public NextPage execute(HttpServletRequest request, HttpServletResponse response) {

		System.out.println("2. AnimalNewService Start");

		Recommend rec = new Recommend();
		DateToday dateToday = new DateToday();

		Time_now timeNow = dateToday.getDate();
		rec.setBgnde(timeNow.getSimple_before().replace("-", ""));
		...
		// 4-1. AnimalNewService성공
		// 세션에 animalList를 저장
		// 다음 url : /jsp/index.jsp
		if (animalList != null) {
			request.getSession().setAttribute("animalListNew", animalList);
			request.getSession().setAttribute("Recommend", rec);

			nextPage.setPageName(request.getContextPath() + "/jsp/index.jsp");
			nextPage.setRedirect(true);
		}

		// 4-2. AnimalNewService실패
		else {
