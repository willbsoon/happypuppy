                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <% //세션영역의 postList 파라미터 받아오기
                                ArrayList<Post> postList=(ArrayList<Post>)request.getSession().getAttribute("postList");%>
                                <tbody>
                                    <%for(int i=0; i<postList.size();i++){ %>
                                   <tr class="even gradeC">
                                        <td><%=postList.get(i).getPostNo() %></td>
                                       <!--  제목을 클릭하면 내용을 보여주는 서비스로 이동 -->
                                        <td><a href="postRead.puppy?postNo=<%=postList.get(i).getPostNo()%>"><%=postList.get(i).getTitle()%></a></td>
                                        <td><%=postList.get(i).getWriteDt() %></td>
                                        <td class="center"><%=postList.get(i).getName()%></td>
                                        <td class="center"><%=postList.get(i).getHitCt() %></td>
                                    </tr>
                                    <%} %>
                                </tbody>
                                <%-- <%// 받아온후 세션영역의 posoList 제거
                                request.getSession().removeAttribute("postList");
                                %> --%>
                            </table><%if(session.getAttribute("loginedUser")!=null){ %>
                            <p align="center">
                            <button type="button" class="btn btn-info" onClick="self.location='./postwrite.jsp';">글쓰기</button>
							</p>
							<%} %>
                            <!-- /.table-responsive -->
                            <!-- <div class="well">
