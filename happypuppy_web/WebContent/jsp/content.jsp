				...
				<table class="table table-striped table-bordered">
					<thead>
						...
						<tr>
							...
							<td colspan="2">
								<!-- 파일 다운로드 기능 --> <a href="">${article.fileName}</a> <%-- <a href="/bbs/download.bbs?fileName=${article.fileName}">${article.fileName}</a> --%>
							</td>
						</tr>
					</thead>
					<tbody>
						...
						<tr>
							<th>첨부</th>
							<td colspan="3"><c:if test="${article.fileName == null}">없음</c:if>
					...
					<c:if test="${id != null}">
						...
						<c:if test="${id == article.id}">
							<input type="button" class="btn btn-default" value="수정하기"
								onclick="document.location.href='/bbs/updateForm.bbs?articleNumber=${article.articleNumber}&pageNum=${pageNum}'">
							<input type="button" class="btn btn-default" value="삭제하기"
								onclick="document.location.href='/bbs/delete.bbs?articleNumber=${article.articleNumber}&pageNum=${pageNum}'">
						</c:if>
						<c:if test="${id != article.id}">
							<input type="button" class="btn btn-default" value="수정하기"
								disabled="disabled">
